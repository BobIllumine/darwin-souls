...
class Agent():
    ...
    # Get Q-function
    def ensemble_q(self, state, skill):
        with torch.no_grad():
            return (self.online_net(state.unsqueeze(0), skill.unsqueeze(0)) * self.support).sum(2)
    ...
    def get_target_q(self, next_states, next_skills):
        with torch.no_grad():
            pns = self.online_net(next_states, next_skills)
            dns = self.support.expand_as(pns) * pns
            argmax_indices_ns = dns.sum(2).argmax(1)
            pns = self.target_net(next_states, next_skills)
            pns_a = pns[range(self.batch_size), argmax_indices_ns]
            pns_a = pns_a * self.support.expand_as(pns_a)            
        return pns_a.sum(1)
    
    # Compute Q-value
    def get_online_q(self, states, skills):
        with torch.no_grad():
            pns = self.online_net(states.unsqueeze(0), skills.unsqueeze(0))
            dns = self.support.expand_as(pns) * pns
        return dns.sum(2)

    def learn(self, mem):
        # Sample transitions
        idxs, states, skills, actions, returns, next_states, next_skills, nonterminals, weights = mem.sample(self.batch_size)
        ...
        with torch.no_grad():
            ...
            n = skills.new_zeros(self.batch_size, self.atoms)
            ...
            n.view(-1).index_add_(0, (l + offset).view(-1), (pns_a * (u.float() - b)).view(-1))  # m_l = m_l + p(s_t+n, a*)(u - b)
            n.view(-1).index_add_(0, (u + offset).view(-1), (pns_a * (b - l.float())).view(-1))  # m_u = m_u + p(s_t+n, a*)(b - l)

        loss = -torch.sum(m * n * log_ps_a, 1)  # Cross-entropy loss (minimises DKL(m||p(s_t, a_t)))
    ...
    def ensemble_learn(self, idxs, states, skills, actions, returns, next_states, next_skills, nonterminals, weights, masks, weight_Q=None):
        # Calculate current state probabilities (online network noise already sampled)
        log_ps = self.online_net(states, skills, log=True)  # Log probabilities log p(s_t, ·; θonline)
        log_ps_a = log_ps[range(self.batch_size), actions]  # log p(s_t, a_t; θonline)
