...
#         print('\n', agent, aec.agent_selection, reward)
#         aec.step(action)
...
parser.add_argument('--T-max', type=int, default=int(2e3), metavar='STEPS', help='Number of training steps (4x number of frames)')
...
if args['evaluate']:
    ...
    with trange(1, args['T_max'] + 1, postfix={'suffix':'initial'}) as bar:
        for T in bar:
            ...
            img = torch.Tensor(next_state['observation'][0] if isinstance(next_state, dict) else next_state[0]).to(args['device'])
            array = img[-1].detach().cpu().numpy()

            # Normalize the array values to 0-255 (if necessary)
            # Assuming the tensor values are between 0 and 1
            ...
            # Save the image
            image.save(f'grayscale_image_{T}.png')
            next_state = torch.Tensor(next_state['observation'][1] if isinstance(next_state, dict) else next_state[1]).to(args['device'])
            state = next_state
            state = state.view(state.size(0), -1)
            output = ae(state)
