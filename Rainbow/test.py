...
# Test DQN
def test(env: UnityAECEnv, agent, args, T, dqn, val_mem, metrics, results_dir, evaluate=False):
    ...
    for _ in range(args['evaluation_episodes']):
        while True:
            ...
            action = dqn.act_e_greedy(state, skill) # Choose an action ε-greedily
            env.step(action)
    ...
    for state, skill in val_mem:  # Iterate over valid states
        T_Qs.append(dqn.evaluate_q(state, skill))
    ...
    if not evaluate:
        ...
        if avg_reward > metrics[agent]['best_avg_reward']:
            ...
            dqn.save(results_dir, name=f'{name_agent}_model.pth')
...
def _plot_line(xs, ys_population, title, path=''):
    ...
    }, filename=os.path.join(path, title + '.png'), auto_open=False)